/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package egg;
/*18. Realizar un programa que rellene un matriz de 4 x 4 de valores aleatorios y muestre la
traspuesta de la matriz. La matriz traspuesta de una matriz A se denota por B y se
obtiene cambiando sus filas por columnas (o viceversa).*/
public class ejercicio18 {
    public static void main(String[] args) {
        int [][] matriz = new int[4][4];
        int [][] trans = new int [4][4];
        rellenarMatriz(matriz);
        imprimir(matriz); //matriz original
        transpuesta(trans, matriz);
        System.out.println("");
        imprimir(trans); //matriz transpuesta
    }
    //Rellena la matriz
    private static void rellenarMatriz(int [][] matriz){
        for (int i = 0; i < matriz.length; i++) {
            for (int j = 0; j < matriz.length; j++) {
                matriz[i][j]=(int)(Math.random()*10);// numero random
            }
        }
    }
    //da la vuelta
    private static void transpuesta(int [][] trans, int [][] matriz){
        for (int i = 0; i < trans.length; i++) {
            for (int j = 0; j < trans.length; j++) {
                trans[i][j]= matriz[j][i];  //transpuesta
            }
        }
    }
    //imprime la matriz
    private static void imprimir(int [][] matriz){
        for (int i = 0; i < matriz.length; i++) {
            for (int j = 0; j < matriz.length; j++) {
                System.out.print(matriz[i][j] + " ");
            }
        System.out.println(" ");
        }
    }
}
